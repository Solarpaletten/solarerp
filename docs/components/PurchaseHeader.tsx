...
function Field({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <div>
      <dt className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-0.5">{label}</dt>
    ...
    <div className={`bg-white border border-gray-200 rounded-lg ${isCancelled ? 'opacity-75' : ''}`}>
      <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h2 className="font-mono text-base font-bold text-gray-900">{purchase.series}-{purchase.number}</h2>
          <span className="text-xs text-gray-400">Purchase Document</span>
        </div>
        <span className={`inline-block px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded border ${statusConfig.bg} ${statusConfig.text} ${statusConfig.border}`}>
          {statusConfig.label}
        </span>
      </div>
      <div className="px-5 py-4 grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-4">
        ...
        <div className="space-y-3">
          ...
          {purchase.comments && (
            <div>
              <dt className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-0.5">Comments</dt>
              <dd className="text-sm text-gray-600 italic">{purchase.comments}</dd>
            </div>
          )}
        </div>
      </div>
