...
// ═══════════════════════════════════════════════════
// Task 44: ERP Grid Engine (v2 — Daria audit patches)
// ═══════════════════════════════════════════════════
// Fixes:
//   #1 T extends { id: string } — typed row id
//   #2 Native debounce (400ms) — no extra deps
//   #3 Sort type guard (already API-side)
//   #4 Sort triggers page reset
//   #5 Checkbox stopPropagation on td
...
export function ERPGrid<T extends { id: string } & Record<string, unknown>>({
  ...
  const search = useDebounce(searchInput, 400);        // Fix #2: debounce
  ...
  // Reset page when search changes (debounced value)
  ...
  const handleSort = (key: string) => {
    ...
    setPage(1);   // Fix #4: reset page on sort
  };
  ...
  const toggleSelectAll = () => {
    if (selected.size === data.length) {
      ...
      setSelected(new Set(data.map(r => r.id)));  // Fix #1: typed id
    }
  };
        ...
        {error ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {data.map((row, idx) => {
                const rowId = row.id;  // Fix #1: typed
                ...
                return (
                  <tr key={rowId}
                    ...
                    {/* Fix #5: stopPropagation on checkbox td */}
              ...
              return (
                <button key={p} onClick={() => setPage(p)}
                  className={`px-2 py-1 text-xs rounded ${
                    page === p ? 'bg-blue-600 text-white font-semibold' : 'text-gray-500 hover:bg-gray-100'
                  }`}>
                  {p}
                </button>
              );
