...
//
// This service is called INSIDE a prisma.$transaction()
// so that if journal creation fails, the document is rolled back.
//
// Task 24: Period Locking integrated.
// Before creating any JournalEntry, checks if period is open.
// If closed → throws PERIOD_CLOSED → transaction rollback.

import { Prisma, PrismaClient } from '@prisma/client';
import { assertPeriodOpen } from './periodGuard';
...
//
// Task 24: Now checks period lock BEFORE creating entry
export async function createJournalEntry(
  tx: TxClient,
  input: CreateJournalEntryInput
) {
  // *** Task 24: Period Lock Check ***
  // If period is closed → throws PERIOD_CLOSED → transaction rollback
