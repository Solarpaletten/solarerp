Аудит принят. Один конкретный фикс — `useRef` guard в `/new` страницах чтобы убрать двойной вызов draft из React StrictMode.## StrictMode Double-Call Fix — 2 файла

```bash
cp sales-new-page.tsx     app/(dashboard)/company/[companyId]/sales/new/page.tsx
cp purchases-new-page.tsx app/(dashboard)/company/[companyId]/purchases/new/page.tsx
```

Добавлен `useRef(false)` guard. В dev mode React StrictMode вызывает `useEffect` дважды — без guard это создавало 2 draft'а, второй падал на unique constraint. Retry спасал, но лог был грязный. Теперь draft создаётся ровно 1 раз.

Task 50 закрыт — 9.2/10. Торговый цикл замкнут. Жду сигнал на Task 51.
