        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
              ...
              <th className="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-400 uppercase min-w-[80px]">Code</th>
              ...
              <th className="px-3 py-2.5 text-right text-[10px] font-semibold text-gray-400 uppercase min-w-[80px]">VAT Amount</th>
            ...
            {items.map((item, index) => {
              ...
              return (
                <tr key={item.id} className="hover:bg-gray-50/50">
                  <td className="px-3 py-2 text-center text-gray-400 tabular-nums">{index + 1}</td>
                  <td className="px-3 py-2 text-gray-900">{item.itemName}</td>
                  <td className="px-3 py-2 text-gray-500 font-mono text-[11px]">{item.itemCode || <span className="text-gray-300">—</span>}</td>
                  <td className="px-3 py-2 text-right text-gray-700 tabular-nums">{fmtQty(qty)}</td>
                  <td className="px-3 py-2 text-right text-gray-700 font-mono tabular-nums">{fmt(price)}</td>
                  <td className="px-3 py-2 text-right text-gray-500 tabular-nums">{vatRate > 0 ? `${fmt(vatRate, 0)}%` : <span className="text-gray-300">—</span>}</td>
                  <td className="px-3 py-2 text-right text-gray-700 font-mono tabular-nums">{fmt(netTotal)}</td>
                  <td className="px-3 py-2 text-right text-gray-500 font-mono tabular-nums">{fmt(vatAmount)}</td>
                  <td className="px-3 py-2 text-right text-gray-900 font-semibold font-mono tabular-nums">{fmt(grossTotal)}</td>
                </tr>
              );
            })}
          </tbody>
